...
class SermonDownloadService {
    ...
    downloadAsWord(sermon) {
        try {
            // Create Word document content
            let content = `${sermon.title}\n\n`;
            if (sermon.scripture)
                content += `Pasaje Bíblico: ${sermon.scripture}\n`;
            if (sermon.topic)
                content += `Tema: ${sermon.topic}\n`;
            if (sermon.date)
                content += `Fecha: ${sermon.date}\n`;
            if (sermon.pastor)
                content += `Pastor: ${sermon.pastor}\n`;
            if (sermon.church)
