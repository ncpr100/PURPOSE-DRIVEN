...
> = {
  ...
  CANCELLED: {
    color: "#F0B83C",
    bg: "rgba(240,184,60,0.06)",
    border: "rgba(240,184,60,0.25)",
    icon: ShieldAlert,
    label: "Cascada",
  },
  UNPROTECTED: {
    color: "#E84855",
    bg: "rgba(232,72,85,0.08)",
    border: "rgba(232,72,85,0.25)",
    icon: ShieldX,
    label: "Sin cobertura",
  },
  UNCONFIRMED: {
...
function CoverageRow({ slot }: { slot: CoverageSlot }) {
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLDivElement).style.borderColor =
          cfg.border || "rgba(255,255,255,0.04)";
      }}
    >
      {/* Status dot */}
      <div
      ...
      <div
        style={{ fontSize: "11px", color: "hsl(var(--foreground))", flex: 1 }}
      >
        {slot.name}
      </div>
      ...
      >
        <div
          style={{
            ...
            background: cfg.color,
            width:
              slot.status === "CONFIRMED" || slot.status === "COVERED"
                ? "100%"
...
function CascadeTimeline({
  ...
  volunteerName,
  role,
}: NonNullable<Props["activeCascade"]>) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLocalSteps((prev) => {
        ...
        if (activeIdx === -1 || waitIdx === -1 || Math.random() > 0.35)
          return prev;
        return prev.map((s, i) => ({
          ...s,
          ...
          >
            ...
            <div
              style={{
                ...
                ...(step.state === "done"
                  ? {
                      background: "rgba(29,201,140,0.2)",
                      borderColor: "#1DC98C",
                      color: "#1DC98C",
                    }
                  : step.state === "active"
                    ? {
                        background: "rgba(240,184,60,0.2)",
                        borderColor: "#F0B83C",
                        color: "#F0B83C",
                        animation: "cascadeActive 1.2s ease-in-out infinite",
                      }
                    : {
            ...
            >
              {step.label}
              <br />
              <span style={{ opacity: 0.7 }}>{step.sublabel}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: "1px",
          background:
            "linear-gradient(90deg, transparent, #C9922A66, transparent)",
        }}
      />
      ...
      >
        ...
        >
          <Shield size={14} style={{ color: "#F0B83C" }} /> Agente 12 —
          Cobertura Dominical
        </div>
        <div
          style={{
        ...
        >
          <span
            style={{
              ...
              fontWeight: 700,
              color:
                rate >= 90 ? "#1DC98C" : rate >= 70 ? "#F0B83C" : "#E84855",
            }}
          >
            {rate}%
          </span>
          <div
            style={{
              ...
              padding: "3px 8px",
              borderRadius: "10px",
              background: "rgba(29,201,140,0.1)",
              color: "#1DC98C",
      ...
      {/* Coverage slots */}
      {slots.map((slot) => (
        <CoverageRow key={slot.id} slot={slot} />
      ))}
