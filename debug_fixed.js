...
async function checkTotalCount() {
  ...
  try {
    // Total members (all churches, all statuses)
    ...
    if (churches.length > 0) {
      ...
      console.log('\nGender distribution (active members only):')
      genderCounts.forEach(g => console.log(`${g.gender || 'null'}: ${g._count.gender}`))
      
      // Total of gender counts should match total active
      const genderTotal = genderCounts.reduce((sum, g) => sum + g._count.gender, 0)
      console.log(`\nGender total: ${genderTotal}`)
      console.log(`Should equal active total: ${totalChurchActive}`)
      console.log(`Match: ${genderTotal === totalChurchActive ? '✅' : '❌'}`)
    }
