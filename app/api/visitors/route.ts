  ...
  try {
    ...
    if (category) {
      ...
      switch (category) {
        ...
        case "RETURNING":
          ...
          break;
        case "REGULAR":
          where.visitorType = "REGULAR";
          break;
        case "MEMBER_CANDIDATE":
          where.visitorType = "MEMBER_CANDIDATE";
          break;
        case "NON_MEMBER":
          where.visitorType = "NON_MEMBER";
          break;
      }
    }
    ...
    const visitors = await Promise.all(
      rawVisitors.map(async (v) => {
        ...
        if (v.visitorType === "MEMBER_CANDIDATE")
          displayCategory = "CANDIDATO A MIEMBRO";
        ...
        const openFollowUps = v.visitor_follow_ups.filter(
          (f) => f.status === "PENDIENTE" || f.status === "pending",
        ).length;
        const closedFollowUps = v.visitor_follow_ups.filter(
    ...
    const filteredVisitors = hasFollowUp
      ? visitors.filter((v) =>
          hasFollowUp === "open"
            ? v.openFollowUps > 0
            : hasFollowUp === "done"
              ? v.openFollowUps === 0 && v.closedFollowUps > 0
              : true,
        )
      : visitors;
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error(
        "[POST /api/visitors] automation failed (non-blocking):",
        automationErr,
      );
    }
