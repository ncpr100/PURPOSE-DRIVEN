...
export async function GET(request: NextRequest) {
  try {
    ...
    const churchInsights = await generateChurchWideAIInsights(
      churchId,
      aiEngine,
    );
    ...
    if (includeAccuracy) {
      ...
      const improvements =
        await accuracyEngine.generateImprovementRecommendations();
      ...
      response.accuracyMetrics = {
        ...
        enhancedCapabilities: {
          ...
          realTimeCalibration:
            "Confidence scores automatically adjust to maintain accuracy",
          adaptiveWeighting:
            "Feature importance dynamically optimized per church context",
          predictionTracking:
            "All predictions tracked and validated against actual outcomes",
        },
      };
      ...
      if (accuracyMetrics.overall < 80) {
        ...
        response.modelOptimization = {
          ...tuningResults,
          message:
            "Model automatically optimized based on performance feedback",
        };
      }
    }
    ...
    response.enhancedCapabilities = {
      ...
      spiritualIntegration: {
        giftsBasedMatching:
          "Deep integration with 8-category spiritual gifts assessment",
        maturityAssessment: "Spiritual maturity factors in all recommendations",
        callingAlignment:
          "Ministry recommendations align with spiritual calling",
      ...
      businessIntelligence: {
        retentionOptimization:
          "Proactive member retention with intervention strategies",
...
async function generateChurchWideAIInsights(
  churchId: string,
  aiEngine: EnhancedAIInsightsEngine,
) {
  return {
    overallHealth: {
      score: 82,
