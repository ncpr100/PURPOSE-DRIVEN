...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Use fallback data
      enhancedData = {
        conversionFunnel: { stageDistribution: {} },
        engagementDistribution: {},
        stageProgression: [],
        retentionAnalytics: { predictiveAccuracy: 0 },
      };
      conversionAnalytics = {};
    ...
    const response: EnhancedMemberJourneyResponse = {
      ...
      enhancedAnalytics: {
        ...
        engagementMetrics: {
          ...
          overallScore: calculateOverallEngagementScore(
            enhancedData.engagementDistribution,
          ),
        },
        behavioralInsights: {
        ...
        recommendationEngine: {
          systemRecommendations: await generateSystemRecommendations(
            churchId,
            enhancedData,
          ),
      ...
      totalMembersAnalyzed: await db.member_journeys.count({
        where: { churchId },
      }),
    };
...
async function getLegacyAnalytics(churchId: string, period: number) {
  ...
  return {
    ...
    spiritualGrowth: calculateSpiritualGrowth(
      members,
      volunteers,
      prayer_requestss,
    ),
...
function calculateOverallEngagementScore(distribution: any): number {
  ...
  return totalMembers > 0
    ? Math.round((totalScore / (totalMembers * 5)) * 100)
    : 0;
}
...
function identifyCommonPatterns(stageProgression: any[]): any[] {
  return stageProgression
    ...
    .map((p) => ({
      ...
      recommendation: generatePatternRecommendation(
        p.progression,
        p.averageDuration,
      ),
    }));
...
async function generateSystemRecommendations(
  ...
  const highRiskCount =
    data.retentionAnalytics.riskDistribution["VERY_HIGH"] || 0;
  if (highRiskCount > 0) {
  ...
  const conversionValues = Object.values(conversionRates).filter(
    (rate): rate is number => typeof rate === "number",
  );
  const lowestConversion =
    conversionValues.length > 0 ? Math.min(...conversionValues) : 0;
...
async function getMemberSpecificActions(churchId: string): Promise<any[]> {
  try {
    ...
    return member_journeyss.map((journey) => ({
      ...
      memberName: journey.members
        ? `${journey.members.firstName} ${journey.members.lastName}`
        : "Unknown",
      issue:
        journey.retentionRisk === "VERY_HIGH"
          ? "High retention risk"
          : journey.engagementScore < 30
            ? "Low engagement"
            : "Stagnated progress",
      recommendedAction:
        journey.recommendedActions &&
        typeof journey.recommendedActions === "string"
          ? JSON.parse(journey.recommendedActions)[0]?.title ||
            "Schedule follow-up"
          : "Schedule follow-up",
...
function calculateConversionFunnel(
  ...
  const leaders = members.filter(
    (m) => m.role && ["PASTOR", "LIDER", "ADMIN_IGLESIA"].includes(m.role),
  ).length;
  ...
  const createStage = (index: number, name: string): MemberJourneyStage => ({
    ...
    count: stages[index],
    percentage:
      totalVisitors > 0 ? Math.round((stages[index] / totalVisitors) * 100) : 0,
    ...
    conversionRate:
      index < stages.length - 1 && stages[index] > 0
...
function calculateSpiritualGrowth(
  members: any[],
  volunteers: any[],
  prayer_requestss: any[],
  ...
  // Engagement metrics
  const prayerParticipants = new Set(
    prayer_requestss.map((pr) => pr.members?.id),
  ).size;
  const activeMembers = members.filter((m) => m.isActive).length;
  const prayerParticipationRate =
    activeMembers > 0
  ...
  return {
    ...
    discipleship: {
      ...
      averageProgress:
        members.length > 0
          ? Math.round((membersInDiscipleship / members.length) * 100)
          : 0,
    },
    ministry: {
      totalVolunteers,
      leadershipDevelopment,
      activeMinistries: new Set(volunteers.map((v) => v.volunteer?.category))
        .size,
    },
    engagement: {
...
function calculatePathwayAnalysis(
  visitors: any[],
  members: any[],
  check_ins: any[],
) {
  ...
  const demographics = [
    {
      ...
      count: members.filter((m) => {
        ...
        const age =
          new Date().getFullYear() - new Date(m.birthDate).getFullYear();
