...
export async function GET(request: Request) {
  try {
    ...
    const cached = await cacheManager.getRetentionAlerts(churchId);
    if (cached) {
    ...
    const highRiskMembers = await db.member_journeys.findMany({
      ...
      orderBy: [{ retentionScore: "asc" }, { lastAnalysisDate: "desc" }],
    });
    ...
    const totalActiveMembers = riskDistribution.reduce(
      (sum, risk) => sum + risk._count,
      0,
    );
    ...
    const trendsByMonth = retentionTrends.reduce(
      (acc, record) => {
        const monthKey = record.lastAnalysisDate.toISOString().substring(0, 7); // YYYY-MM
        if (!acc[monthKey]) {
          acc[monthKey] = {
            scores: [],
            risks: { HIGH: 0, MEDIUM: 0, LOW: 0, VERY_HIGH: 0 },
          };
        }
    ...
    const monthlyTrends = Object.entries(trendsByMonth).map(
      ([month, data]: [string, MonthlyRetentionData]) => ({
        month,
    ...
    const riskStats = {
      critical:
        riskDistribution.find((r) => r.retentionRisk === "VERY_HIGH")?._count ||
        0,
      high:
        riskDistribution.find((r) => r.retentionRisk === "HIGH")?._count || 0,
      medium:
        riskDistribution.find((r) => r.retentionRisk === "MEDIUM")?._count || 0,
    ...
    const retentionAlertData = {
      ...
      riskDistribution: [
        {
          ...
          percentage: Math.round(
            (riskStats.critical / totalActiveMembers) * 100,
          ),
        },
        {
          level: "HIGH",
          count: riskStats.high,
          percentage: Math.round((riskStats.high / totalActiveMembers) * 100),
        },
        {
          level: "MEDIUM",
          count: riskStats.medium,
          percentage: Math.round((riskStats.medium / totalActiveMembers) * 100),
        },
        {
          level: "LOW",
          count: riskStats.low,
          percentage: Math.round((riskStats.low / totalActiveMembers) * 100),
        },
      ],
      ...
      interventionStrategies: {
        urgent: alertsWithStrategies.filter(
          (alert) => alert.intervention.priority === "critical",
        ).length,
        high: alertsWithStrategies.filter(
          (alert) => alert.intervention.priority === "high",
        ).length,
        medium: alertsWithStrategies.filter(
          (alert) => alert.intervention.priority === "medium",
        ).length,
        low: alertsWithStrategies.filter(
          (alert) => alert.intervention.priority === "low",
        ).length,
      },
      recommendations: [
        {
        ...
        },
        {
          ...
          description:
            "Desarrollar programa preventivo para miembros en riesgo medio",
    ...
    // Line ~322
    await cacheManager.cacheRetentionAlerts(churchId, retentionAlertData);
