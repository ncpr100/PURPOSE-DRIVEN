...
async function cleanupOldCommunications(churchId: string) {
  ...
  console.log(
    `Data retention cleanup: Deleted ${deletedCount.count} old communications for church ${churchId}`,
  );
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `Communication API POST request at ${new Date().toISOString()}`,
    );
    ...
    console.log(
      `Communication created: ${communication.id} by user ${session.user.id}`,
    );
