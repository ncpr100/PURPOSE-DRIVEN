...
async function optimizeImageForPlatforms(
  ...
  for (const platform of platforms) {
    try {
      ...
      const spec = requirements[contentType as keyof typeof requirements] as
      ...
      results.push({
        ...
        compressionRatio: Math.round(
          (1 - optimizedBuffer.length / imageBuffer.length) * 100,
        ),
      });
    } catch (error) {
...
async function optimizeVideoForPlatforms(
  ...
  for (const platform of platforms) {
    try {
      ...
      const isValidFormat = requirements.formats.includes(
        upperExtension as any,
      );
