  ...
  try {
    ...
    const syncResults = await Promise.allSettled(
      connectedAccounts.map((account) =>
        syncAccountAnalytics(account, forceSync),
      ),
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    accounts.forEach((account) => {
      ...
      const platformMetrics = platformPosts.reduce(
        (acc, post) => {
          acc.posts += 1;
          const engagement =
            typeof post.engagement === "string"
    ...
    const topPosts = posts
      .map((post) => {
        ...
        const totalEngagement =
          typeof engagement === "number" ? engagement : engagement.total || 0;
    ...
    const engagementTrends = await calculateEngagementTrends(
      churchId,
      dateFrom,
      dateTo,
    );
        ...
        summary: {
          ...
          engagementRate:
            totalMetrics.impressions > 0
              ? (
                  (totalMetrics.engagement / totalMetrics.impressions) *
                  100
                ).toFixed(2)
              : "0.00",
        },
...
async function syncAccountAnalytics(account: any, forceSync: boolean = false) {
  try {
    ...
    console.error(
      `Analytics sync failed for ${account.platform} account ${account.id}:`,
      error,
    );
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      const totalEngagement =
        typeof engagement === "number" ? engagement : engagement.total || 0;
      const impressions =
        typeof engagement === "object" ? engagement.impressions || 0 : 0;
    ...
    const trends = Object.values(dailyMetrics).map((day: any) => ({
      ...
      engagementRate:
        day.impressions > 0
          ...
          : "0.00",
      averageEngagement:
        day.posts > 0 ? Math.round(day.engagement / day.posts) : 0,
    }));
