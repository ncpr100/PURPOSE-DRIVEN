...
const broadcastSchema = z.object({
  type: z
    .enum(["notification", "system", "chat", "presence"])
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !["SUPER_ADMIN", "ADMIN_IGLESIA", "PASTOR"].includes(sessionUser.role)
    ) {
      return NextResponse.json(
        { error: "Sin permisos para enviar notificaciones en tiempo real" },
        { status: 403 },
      );
    }
    ...
    const realtimeMessage = {
      ...
      userId:
        validatedData.target === "user" ? validatedData.targetId : undefined,
      churchId: sessionUser.churchId,
      role:
        validatedData.target === "role" ? validatedData.targetId : undefined,
      timestamp: Date.now(),
    };
    ...
    switch (validatedData.target) {
      case "user":
        if (!validatedData.targetId) {
          return NextResponse.json(
            { error: 'targetId requerido para target "user"' },
            { status: 400 },
          );
        }
      ...
      case "church":
        ...
        if (
          ...
          targetChurchId !== sessionUser.churchId
        ) {
          return NextResponse.json(
            { error: "Sin permisos para enviar a esta iglesia" },
            { status: 403 },
          );
        }
      ...
      case "role":
        if (!validatedData.targetId) {
          return NextResponse.json(
            { error: 'targetId (role) requerido para target "role"' },
            { status: 400 },
          );
        }
      ...
      case "global":
        ...
        if (sessionUser.role !== "SUPER_ADMIN") {
          return NextResponse.json(
            { error: "Solo SUPER_ADMIN puede hacer broadcasts globales" },
            { status: 403 },
          );
        }
      ...
      default:
        return NextResponse.json(
          { error: "Tipo de target no válido" },
          { status: 400 },
        );
    }
    ...
    if (!success && validatedData.target === "user") {
      return NextResponse.json({
        success: false,
        message:
          "Usuario no está conectado, notificación no enviada en tiempo real",
        realtimeMessage,
      });
    }
