...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          availableChurches: await db.churches.findMany({
    ...
    // Check existing data
    const existingMembers = await db.members.count({
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      for (let i = 0; i < currentBatchSize; i++) {
        ...
        if (rand < 0.1) lifecycle = "VISITANTE";
        else if (rand < 0.3) lifecycle = "NUEVO_CREYENTE";
        else if (rand < 0.65) lifecycle = "CRECIMIENTO";
        else if (rand < 0.9) lifecycle = "MADURO";
        ...
        members.push({
          ...
          joinDate,
          baptismDate:
            lifecycle !== "VISITANTE" && Math.random() > 0.3
              ? randomDate(joinDate, new Date())
              : null,
          churchId: church.id,
          createdAt: joinDate,
          updatedAt: new Date(),
      ...
      console.log(
        `[SEED-HILLSONG] Batch ${batch + 1}/${batches} completed (${members.length} members)`,
      );
    }
...
export async function GET() {
  return NextResponse.json({
    message:
      "POST to this endpoint to seed Hillsong Barranquilla with 2000 members",
    willCreate: {
