...
export async function GET(request: NextRequest) {
  try {
    ...
    if (
      ...
      session.user.role !== "ADMIN_IGLESIA"
    ) {
    ...
    switch (action) {
      case "status":
        const optimizationStatus =
          await cacheOptimizationController.getOptimizationStatus();
  ...
  try {
    ...
    switch (action) {
      ...
      case "add_target":
        if (!target) {
          return NextResponse.json(
            { error: "Target configuration is required" },
            { status: 400 },
          );
        }
      ...
      default:
        return NextResponse.json(
          {
            error:
              "Invalid action. Use: force_optimization, add_target, remove_target, or clear_cache",
          },
          { status: 400 },
        );
    }
