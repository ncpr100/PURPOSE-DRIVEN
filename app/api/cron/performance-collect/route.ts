...
export async function GET(req: NextRequest) {
  ...
  if (!agent?.isEnabled) {
    ...
    return NextResponse.json({
      skipped: true,
      reason:
        "Agent 13 (Performance Engineer) is disabled in platform settings",
    });
  }
  ...
  try {
      ...
      if (!existing) {
        await db.platform_incidents.create({
          data: {
            ...
            // ✅ FIX: Ensure 'string' type by providing fallbacks for undefined values
            title: `Performance: ${anomaly.type || "Unknown Service"}`,
            description: anomaly.description || "No description available",
            // ✅ FIX: Ensure array contains only strings
            affectedServices: [anomaly.type || "Unknown Service"],
            detectedAt: new Date(),
            status: "DETECTED",
          },
        });
    ...
    // ✅ FIX: Pass snapshot first, then anomalies array with type assertion
    if (anomalies.length > 0) {
      const recommendations = await generatePerformanceRecommendations(
        snapshot,
        anomalies as any, // Temporary assertion to bypass strict type mismatch
      );
