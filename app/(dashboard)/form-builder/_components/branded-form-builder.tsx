...
export default function BrandedFormBuilder({ churchId }: { churchId?: string }) {
  ...
  const [rightPanel, setRightPanel] = useState<'properties' | 'style'>('properties')
  ...
  const insertBlock = (type: FormField['type']) => {
    const newField: FormField = {
      id: Date.now(),
      label: type === 'heading' ? 'Nuevo Encabezado'
            : type === 'divider' ? ''
            : type === 'email' ? 'Correo Electrónico'
            : type === 'tel' ? 'Teléfono'
            : type === 'textarea' ? 'Comentarios'
            : type === 'select' ? 'Selecciona una opción'
            : type === 'checkbox' ? 'Aceptar términos'
            : type === 'radio' ? 'Escoge una opción'
            : type === 'date' ? 'Fecha'
            : type === 'number' ? 'Número'
            : 'Nuevo Campo',
      type,
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-none bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          {!showTemplates ? (
            <Button
              onClick={() => { setShowTemplates(true); setSelectedFieldId(null) }}
              variant="ghost"
              size="sm"
          ...
          <div>
            {!showTemplates ? (
              <div className="flex items-center gap-3">
                <span className="font-semibold text-gray-800 text-sm">{formConfig.title}</span>
              ...
              <h1 className="text-lg font-bold text-gray-800">Plantillas de Formularios</h1>
            )}
          </div>
        </div>
      ...
      {showTemplates ? (
        <div className="flex-1 overflow-auto p-6">
          <div className="max-w-5xl mx-auto">
            <p className="text-gray-500 mb-6 text-sm">Elige una plantilla para empezar o crea desde cero.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Card
                ...
                onClick={() => {
                  updateFormConfig(prev => ({
                    ...
                    description: '',
                    fields: [{ id: Date.now(), label: 'Nombre Completo', type: 'text', required: true, options: [] }]
                  }))
              ...
              >
                <CardContent className="flex flex-col items-center justify-center py-10 text-gray-500">
                  ...
                  <span className="text-xs text-gray-400 mt-1">Empieza desde cero</span>
                </CardContent>
              </Card>

              {SMART_TEMPLATES.map((template) => (
                <Card key={template.id} className="hover:shadow-lg transition-all cursor-pointer border border-gray-200" onClick={() => applyTemplate(template)}>
                  ...
                  <CardContent className="pt-0">
                    ...
                    <span className="text-xs text-gray-400">{template.fields.length} campos</span>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-52 flex-none bg-white border-r border-gray-200 overflow-y-auto flex flex-col">
            <div className="p-3 border-b border-gray-100">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Bloques</p>
            </div>

            {/* QUESTIONS */}
            <div className="p-2">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-2 py-1">Preguntas</p>
              {[
                { type: 'text' as const,     label: 'Respuesta Corta',  Icon: Type },
                { type: 'textarea' as const, label: 'Respuesta Larga',  Icon: AlignLeft },
                { type: 'select' as const,   label: 'Desplegable',      Icon: DropdownIcon },
                { type: 'radio' as const,    label: 'Opción Múltiple',  Icon: Circle },
                { type: 'checkbox' as const, label: 'Casilla',          Icon: CheckSquare },
            ...
            {/* LAYOUT BLOCKS */}
            <div className="p-2">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-2 py-1">Diseño</p>
              {[
            ...
            {/* PRESET QUICK-ADD */}
            <div className="p-2">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-2 py-1">Campos Rápidos</p>
              {PRESET_FIELDS.slice(0, 12).map((preset) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    updateFormConfig(prev => {
                      if (selectedFieldId === null) return { ...prev, fields: [...prev.fields, newField] }
                      const idx = prev.fields.findIndex(f => f.id === selectedFieldId)
                      const insertAt = idx === -1 ? prev.fields.length : idx + 1
                      const updated = [...prev.fields]
                      updated.splice(insertAt, 0, newField)
            ...
            >
              ...
              {formConfig.churchLogo && (
                <div className="flex justify-center pt-6 px-8">
                  <img src={formConfig.churchLogo} alt="Logo" className="h-16 object-contain" />
                </div>
              )}
              ...
              <div className="px-8 pb-4 space-y-1">
                {formConfig.fields.map((field, index) => {
                      ...
                      >
                        ...
                        {isSelected && (
                          <div className="absolute -top-2 right-1 flex gap-1">
                            <button onClick={(e) => { e.stopPropagation(); moveField(field.id, 'up') }} className="bg-white border rounded p-0.5 hover:bg-gray-100"><ChevronUp className="h-3 w-3" /></button>
                            <button onClick={(e) => { e.stopPropagation(); moveField(field.id, 'down') }} className="bg-white border rounded p-0.5 hover:bg-gray-100"><ChevronDown className="h-3 w-3" /></button>
                            <button onClick={(e) => { e.stopPropagation(); removeField(field.id) }} className="bg-white border border-red-300 rounded p-0.5 hover:bg-red-50"><Trash2 className="h-3 w-3 text-red-500" /></button>
                          </div>
                        )}
                      </div>
                    )
                  ...
                  if (field.type === 'heading') {
                    const Tag = (`h${field.headingLevel || 2}`) as keyof JSX.IntrinsicElements
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          style={{ fontSize: sizes[field.headingLevel || 2], color: formConfig.headerTextColor || '#111827' }}
                          value={field.label}
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="absolute -top-2 right-1 flex gap-1 z-10">
                          <button onClick={(e) => { e.stopPropagation(); moveField(field.id, 'up') }} className="bg-white border rounded p-0.5 hover:bg-gray-100 shadow-sm"><ChevronUp className="h-3 w-3" /></button>
                          <button onClick={(e) => { e.stopPropagation(); moveField(field.id, 'down') }} className="bg-white border rounded p-0.5 hover:bg-gray-100 shadow-sm"><ChevronDown className="h-3 w-3" /></button>
                          <button onClick={(e) => { e.stopPropagation(); removeField(field.id) }} className="bg-white border border-red-300 rounded p-0.5 hover:bg-red-50 shadow-sm"><Trash2 className="h-3 w-3 text-red-500" /></button>
                        </div>
                      )}
                      ...
                      {field.type === 'textarea' ? (
                        <textarea
                          ...
                          style={{ borderColor: formConfig.inputBorderColor || '#d1d5db', borderWidth: '1px', borderStyle: 'solid', borderRadius: formConfig.borderRadius || '8px', fontSize: formConfig.inputFontSize || '14px', color: '#9ca3af', backgroundColor: '#fff' }}
                          rows={3}
                        ...
                        <select
                          ...
                          style={{ borderColor: formConfig.inputBorderColor || '#d1d5db', borderWidth: '1px', borderStyle: 'solid', borderRadius: formConfig.borderRadius || '8px', fontSize: formConfig.inputFontSize || '14px', color: '#6b7280', backgroundColor: '#fff' }}
                        >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span style={{ color: formConfig.bodyTextColor || '#374151', fontSize: formConfig.inputFontSize || '14px' }}>{field.label}{field.required && <span className="text-red-500 ml-1">*</span>}</span>
                        </div>
                      ) : field.type === 'radio' && field.options ? (
                        <div className="space-y-1.5">
                          {field.options.map((opt, i) => (
                            <div key={i} className="flex items-center gap-2">
                              <input type="radio" name={`prev-${field.id}`} className="pointer-events-none" />
                              <span style={{ color: formConfig.bodyTextColor || '#374151', fontSize: formConfig.inputFontSize || '14px' }}>{opt}</span>
                            </div>
                          ))}
                        </div>
                      ) : (
                ...
                <button
                  onClick={() => { setSelectedFieldId(null); insertBlock('text') }}
          ...
          <div className="w-72 flex-none bg-white border-l border-gray-200 overflow-y-auto flex flex-col">
            ...
            {rightPanel === 'properties' ? (
              ...
              <div className="flex-1 p-4 space-y-4">
                {selectedFieldId === null ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-xs text-gray-500 font-medium">FORMULARIO</p>
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Input type="color" value={formConfig.submitButtonColor || '#2563eb'} onChange={(e) => updateFormConfig(prev => ({ ...prev, submitButtonColor: e.target.value }))} className="mt-1 h-8" />
                      </div>
                      <div>
                        <Label className="text-xs">Color Texto</Label>
                        <Input type="color" value={formConfig.submitButtonTextColor || '#ffffff'} onChange={(e) => updateFormConfig(prev => ({ ...prev, submitButtonTextColor: e.target.value }))} className="mt-1 h-8" />
                      </div>
                    </div>

                    <Separator />
                    <p className="text-xs text-gray-500 font-medium">LOGO DE IGLESIA</p>
                    <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 text-center">
                      {formConfig.churchLogo ? (
                        <div>
                          <img src={formConfig.churchLogo} alt="Logo" className="h-12 object-contain mx-auto mb-2" />
                          <Button type="button" variant="ghost" size="sm" onClick={() => updateFormConfig(prev => ({ ...prev, churchLogo: undefined }))} className="text-red-500 text-xs"><X className="h-3 w-3 mr-1" />Quitar</Button>
                        </div>
                      ) : (
                        <>
                          <ImageIcon className="h-8 w-8 text-gray-300 mx-auto mb-1" />
                          <Button type="button" variant="outline" size="sm" onClick={() => churchLogoInputRef.current?.click()} className="text-xs">
                            <Upload className="h-3 w-3 mr-1" />Subir Logo
                          </Button>
                          <input ref={churchLogoInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleImageUpload(f, 'form-background') }} />
                        </>
                      )}
                    </div>

                    <Separator />
                    <p className="text-xs text-gray-400 italic">Haz clic en un campo del formulario para editarlo.</p>
                  </div>
                ) : (() => {
                  const field = formConfig.fields.find(f => f.id === selectedFieldId)
                  if (!field) return <p className="text-xs text-gray-400">Campo no encontrado.</p>

                  return (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-500 font-medium">CAMPO SELECCIONADO</p>
                        <button onClick={() => removeField(field.id)} className="text-red-400 hover:text-red-600 transition-colors">
                      ...
                      {field.type !== 'divider' && (
                        <div>
                          <Label className="text-xs">{field.type === 'heading' ? 'Texto del Encabezado' : 'Etiqueta del Campo'}</Label>
                          <Input
                            value={field.label}
                            onChange={(e) => updateField(field.id, 'label', e.target.value)}
                            className="mt-1 h-8 text-sm"
                      ...
                      {/* Placeholder */}
                      {!['checkbox', 'radio', 'select', 'heading', 'divider', 'date'].includes(field.type) && (
                        <div>
                            ...
                            onValueChange={(val) => {
                              ...
                              if (val === 'select' || val === 'radio') {
                                if (!field.options || field.options.length === 0) {
                                  updateField(field.id, 'options', ['Opción 1', 'Opción 2'])
                                }
                              }
                            }}
                          >
                      ...
                      {field.type === 'heading' && (
                        <div>
                          <Label className="text-xs">Nivel de Encabezado</Label>
                          <Select
                            value={String(field.headingLevel || 2)}
                            onValueChange={(v) => updateField(field.id, 'headingLevel', parseInt(v))}
                          >
                            <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue /></SelectTrigger>
                            <SelectContent>
                      ...
                      {!['heading', 'divider'].includes(field.type) && (
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <label htmlFor={`req-${field.id}`} className="text-sm text-gray-700 cursor-pointer">Campo obligatorio</label>
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const newOpts = [...(field.options || []), `Opción ${(field.options?.length || 0) + 1}`]
                                updateField(field.id, 'options', newOpts)
                              }}
                      ...
                      <div className="flex gap-2">
                        <Button type="button" variant="outline" size="sm" onClick={() => moveField(field.id, 'up')} className="flex-1 h-7 text-xs">
                          <ChevronUp className="h-3 w-3 mr-1" />Subir
                        </Button>
                        <Button type="button" variant="outline" size="sm" onClick={() => moveField(field.id, 'down')} className="flex-1 h-7 text-xs">
                          <ChevronDown className="h-3 w-3 mr-1" />Bajar
                        </Button>
                      </div>
                    </div>
                  )
              ...
              <div className="flex-1 p-4 space-y-5">
                <p className="text-xs text-gray-500 font-medium">COLORES Y TIPOGRAFÍA</p>

                <div className="space-y-3">
                  {[
                    { label: 'Color Título', key: 'headerTextColor', default: '#111827' },
                    { label: 'Color Texto', key: 'bodyTextColor', default: '#374151' },
                    { label: 'Color Principal', key: 'primaryColor', default: '#2563eb' },
                    { label: 'Fondo Formulario', key: 'formBackgroundColor', default: '#ffffff' },
                    { label: 'Borde Inputs', key: 'inputBorderColor', default: '#d1d5db' },
                    { label: 'Foco Inputs', key: 'inputFocusColor', default: '#2563eb' },
                    ...
                    <div key={key} className="flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={(e) => updateFormConfig(prev => ({ ...prev, [key]: e.target.value }))}
                ...
                <Separator />
                <p className="text-xs text-gray-500 font-medium">FUENTE Y TAMAÑOS</p>

                <div>
                  <Label className="text-xs">Familia de Fuente</Label>
                  <Select value={formConfig.fontFamily || 'Inter, sans-serif'} onValueChange={(v) => updateFormConfig(prev => ({ ...prev, fontFamily: v }))}>
                    <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { label: 'Título', key: 'titleFontSize', opts: ['18px', '22px', '28px', '32px', '36px'] },
                    { label: 'Descripción', key: 'bodyFontSize', opts: ['12px', '14px', '16px', '18px'] },
                    { label: 'Etiquetas', key: 'fieldLabelFontSize', opts: ['11px', '12px', '14px', '16px'] },
                    { label: 'Inputs', key: 'inputFontSize', opts: ['12px', '14px', '16px'] },
                  ].map(({ label, key, opts }) => (
                    <div key={key}>
                      <Label className="text-xs">{label}</Label>
                      <Select value={(formConfig as any)[key] || opts[1]} onValueChange={(v) => updateFormConfig(prev => ({ ...prev, [key]: v }))}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Select value={formConfig.borderRadius || '8px'} onValueChange={(v) => updateFormConfig(prev => ({ ...prev, borderRadius: v }))}>
                      <SelectTrigger className="mt-1 h-7 text-xs"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select value={formConfig.formMaxWidth || '640px'} onValueChange={(v) => updateFormConfig(prev => ({ ...prev, formMaxWidth: v }))}>
                      <SelectTrigger className="mt-1 h-7 text-xs"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <Separator />
                <p className="text-xs text-gray-500 font-medium">IMAGEN DE FONDO</p>
                <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 text-center">
                  {formConfig.backgroundImage ? (
                    <div>
                      <img src={formConfig.backgroundImage} alt="Fondo" className="max-h-24 object-cover mx-auto rounded mb-2" />
                      <Button type="button" variant="ghost" size="sm" onClick={() => updateFormConfig(prev => ({ ...prev, backgroundImage: undefined }))} className="text-red-500 text-xs"><X className="h-3 w-3 mr-1" />Quitar</Button>
                    </div>
                  ) : (
                    <>
                      <ImageIcon className="h-8 w-8 text-gray-300 mx-auto mb-1" />
                      <Button type="button" variant="outline" size="sm" onClick={() => formBgInputRef.current?.click()} className="text-xs">
                        <Upload className="h-3 w-3 mr-1" />Subir Fondo
                      </Button>
                      <input ref={formBgInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleImageUpload(f, 'form-background') }} />
                    </>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button onClick={() => setShowQRModal(false)} className="text-gray-400 hover:text-gray-700 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-4">
              {!currentFormSlug && (
                <Alert className="mb-4 border-orange-200 bg-orange-50">
                  ...
                  <AlertDescription className="text-orange-800 text-sm">
                    Guarda el formulario primero para generar un QR con URL corta y escaneable.
                    <Button onClick={async () => { await saveForm(); }} size="sm" className="ml-2 bg-orange-600 hover:bg-orange-700 text-white text-xs h-7">
                      <Save className="h-3 w-3 mr-1" />Guardar Ahora
                    </Button>
                  </AlertDescription>
                </Alert>
              )}
