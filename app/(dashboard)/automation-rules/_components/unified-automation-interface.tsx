...
const getCategoryLabel = (category: string) => {
  const labels: Record<string, string> = {
    'PRAYER_REQUEST': 'Peticiones de OraciÒ³n', 'VISITOR_FOLLOWUP': 'Seguimiento de Visitantes',
...
export function UnifiedAutomationInterface() {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error('Error al cargar datos de automatizaciÒ³n')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Reglas de AutomatizaciÒ³n</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border border-border bg-[hsl(var(--card))]">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Tasa de Ò⬰xito</p>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Configura colores personalizados para cada categorÒ­a de automatizaciÒ³n.
                </p>
              </div>
            </div>
            <Button 
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {rule.lastExecuted && (
                          <span className="flex items-center gap-1">
                            <Clock className="h-4 w-4" />Òšltima: {new Date(rule.lastExecuted).toLocaleDateString('es-ES')}
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3 text-sm">
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground">La automatizaciÒ³n se activa cuando ocurre un evento especÒ­fico (nueva peticiÒ³n de oraciÒ³n, visitante, etc.)</p>
              </div>
              <div className="space-y-2">
                <p className="font-medium">2. Condiciones</p>
                <p className="text-muted-foreground">Se evalÒºan condiciones (ej: categorÒ­a URGENTE, primera visita) para determinar si continuar</p>
              </div>
              <div className="space-y-2">
