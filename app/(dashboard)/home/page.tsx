...
export default async function DashboardPage() {
  ...
  try {
    ...
    recentMembers = await db.members
      .findMany({
        where: {
          churchId: session.user.churchId,
          isActive: true,
    ...
    recentSermons = await db.sermons
      .findMany({
        where: {
          churchId: session.user.churchId,
        },
    ...
    recentCheckIns = await db.check_ins
      .findMany({
        where: {
          churchId: session.user.churchId,
        },
  ...
  if (["PASTOR", "ADMIN_IGLESIA"].includes(session.user.role ?? "")) {
    try {
      const cached = await db.shepherds_log_cache
        .findUnique({
          where: { churchId: session.user.churchId },
        })
        .catch(() => null);
  ...
  const visitorArray = Array.isArray(visitorEngagementData)
  ...
  const averageEngagementScore =
    visitorArray.length > 0
      ? Math.round(
          visitorArray.reduce(
            (sum: number, visitor: any) => sum + (visitor.engagementScore || 0),
            0,
          ) / visitorArray.length,
        )
      : 0;
  ...
  const automationSuccessRate =
    visitorArray.length > 0
      ? Math.round(
          (visitorArray.filter((v: any) => v.automationTriggered).length /
            visitorArray.length) *
            100,
        )
      : 0;

  const returningVisitorsCount = visitorArray.filter(
    (v: any) => v.visitorType === "returning" || !v.isFirstTime,
  ).length;
  const firstTimeVisitorsCount = visitorArray.filter(
    (v: any) => v.isFirstTime,
  ).length;
  ...
  const kpiData = {
    ...
    pendingWebsiteRequests: websiteRequests.filter(
    ...
    followUpCompletionRate:
      pendingFollowUps > 0
        ? Math.round(
            (completedFollowUpsThisMonth /
              (pendingFollowUps + completedFollowUpsThisMonth)) *
              100,
          )
        : 0,
  };
