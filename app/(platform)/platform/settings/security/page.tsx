...
export default function SecuritySettingsPage() {
  ...
  useEffect(() => {
    if (setupData?.otpauthUrl) {
      QRCode.toDataURL(setupData.otpauthUrl, {
        ...
        .then((url) => {
          setQrDataUrl(url);
          console.log("✅ QR Code generado localmente");
        })
        .catch((err) => {
  ...
  const handleVerify = async () => {
    ...
    console.log("🔄 [FRONTEND] Iniciando verificación con código:", totpCode);
    setError("");
    setIsVerifying(true);

    try {
      // Timeout de 10 segundos para evitar carga indefinida
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("✅ [FRONTEND] Datos recibidos del servidor:", data); // ← AGREGAR ESTA LÍNEA
        setSetupData({ secret: data.secret, otpauthUrl: data.otpauthUrl });
        ...
        setSuccess(
          "¡2FA activado exitosamente! Guarda tus códigos de respaldo.",
        );
      } else {
        const errData = await res.json();
        console.error("❌ [FRONTEND] Error en verificación:", errData);
        setError(errData.error || "Código inválido o expirado");
      }
    } catch (err: any) {
      console.error("❌ [FRONTEND] Excepción en verificación:", err);
      if (err.name === "AbortError") {
        setError(
          "La verificación tardó demasiado. Verifica tu conexión e intenta de nuevo.",
        );
      } else {
        setError(
          "Error de red al verificar. Revisa la consola del navegador (F12) para más detalles.",
        );
      }
    } finally {
    ...
    <div className="space-y-6 max-w-2xl mx-auto p-4">
      <div>
        ...
        <p className="text-muted-foreground">
          Gestiona la autenticación de dos factores (2FA) para proteger tu
          cuenta.
        </p>
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          {setupData && (
            <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
              {/* INSTRUCCIONES CON COLORES COSMOS 100% COMPATIBLES */}
                ...
                <ol className="space-y-3 text-sm text-muted-foreground">
                  ...
                  <li className="flex gap-3">
                    ...
                    <span>
                      ...
                      que muestra la app y haz clic en Verificar.
                    </span>
                  </li>
                </ol>
              </div>
          ...
          {backupCodes.length > 0 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
                <h3 className="text-base font-semibold text-yellow-600 dark:text-yellow-400 mb-2">
                  ⚠️ Guarda estos códigos de respaldo
                </h3>
                <p className="text-sm text-yellow-700 dark:text-yellow-300 mb-3">
                  Si pierdes acceso a tu aplicación, usa uno de estos códigos.
                ...
                <div className="grid grid-cols-2 gap-2">
                  {backupCodes.map((code, idx) => (
                    <div
                      key={idx}
                      className="bg-background border border-border rounded p-2 text-center font-mono text-sm text-foreground"
                    >
                      {code}
                    </div>
                  ))}
