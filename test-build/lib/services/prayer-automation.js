...
exports.PrayerAutomation = void 0;
...
class PrayerAutomation {
        ...
        try {
            ...
            for (const rule of automation_ruless) {
                ...
                if (rule.bypassApproval) {
                    ...
                    const pastors = await prisma_1.prisma.users.findMany({
                        where: {
                            churchId: prayer_requests.churchId,
                            role: {
                    ...
                    await prisma_1.prisma.prayer_approvals.create({
                        data: {
     ...
     */
    static async approvePrayerRequest(approvalId, approverId) {
        try {
            // Update approval status
            const approval = await prisma_1.prisma.prayer_approvals.update({
                where: { id: approvalId },
                data: {
                    status: 'approved',
            ...
            const prayerRequest = await prisma_1.prisma.prayer_requests.findUnique({
    ...
    async createFromVisitor(data) {
        console.log('Prayer request created from visitor:', data);
        return 'prayer-request-id';
    }
}
exports.PrayerAutomation = PrayerAutomation;
//# sourceMappingURL=prayer-automation.js.map