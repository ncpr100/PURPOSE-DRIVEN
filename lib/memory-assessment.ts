...
export class MemoryAssessment {
   ...
   */
  async analyzeProjectMemory(): Promise<MemoryReport> {
  ...
  private async scanDirectory(dir: string, relativePath = ''): Promise<string[]> {
    const files: string[] = [];
  ...
  private async analyzeFile(filePath: string, stats: any): Promise<FileAnalysis> {
  ...
  private categorizeFile(fileName: string, filePath: string): FileAnalysis['type'] {
    for (const [type, pattern] of Object.entries(this.filePatterns)) {
      if (pattern.test(fileName)) {
        return type as FileAnalysis['type'];
      }
    }
    return 'other';
  }
   ...
   */
  private determinePriority(fileName: string, relativePath: string, type: string): FileAnalysis['priority'] {
  ...
  private generateReport(analyses: FileAnalysis[]): MemoryReport {
    const filesByType: Record<string, FileAnalysis[]> = {};
    const safeToDelete: FileAnalysis[] = [];
    const requiresReview: FileAnalysis[] = [];
    ...
    for (const file of report.cleanupRecommendations.safeToDelete) {
      ...
      try {
        if (dryRun) {
          ...
          const backupDir = path.join(this.projectRoot, '.cleanup-backup', path.dirname(file.path));
          await fs.mkdir(backupDir, { recursive: true });
          
          // Backup file before deletion
          const backupPath = path.join(this.projectRoot, '.cleanup-backup', file.path);
...
export async function runMemoryAssessment(projectRoot?: string): Promise<MemoryReport> {
  const assessment = new MemoryAssessment(projectRoot);
  return assessment.analyzeProjectMemory();
}

export async function generateMemoryReport(projectRoot?: string): Promise<string> {
  const assessment = new MemoryAssessment(projectRoot);
  return assessment.generateMemoryReport();
}

export async function performSafeCleanup(report: MemoryReport, dryRun = true, projectRoot?: string): Promise<void> {
  const assessment = new MemoryAssessment(projectRoot);
